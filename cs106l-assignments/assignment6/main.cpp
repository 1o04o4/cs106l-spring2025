  ...
  if (argc == 2) {
    ...
    ********************************************************/
    ...
    std::string output = /* STUDENT_TODO */
      course
      .and_then(
        [](const Course& c) -> std::optional<std::string> { 
          return std::format("Found course: {},{},{}", c.title, c.number_of_units, c.quarter);
        }
      )
      .or_else([]() -> std::optional<std::string> { return std::string{"Course not found."}; })
      .value();
